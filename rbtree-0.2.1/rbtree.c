...
static VALUE
rbtree_reverse_each_starting_with_body(rbtree_each_arg_t* arg)
{
    VALUE self = arg->self;
    dict_t* dict = DICT(self);
    dnode_t* node;
...
/*
 * call-seq:
 *   rbtree.each_from_key(key) {|key, value| block} => rbtree
 *
...
VALUE
rbtree_each_from_key(VALUE self, VALUE key)
{
    dnode_t* node = dict_lookup(DICT(self), TO_KEY(key));
    rbtree_each_arg_t each_arg;
...
/*
 * call-seq:
 *   rbtree.reverse_each_from_key(key) {|key, value| block} => rbtree
 *
 * Calls block once for each key in reverse order, starting with the given key,
 * passing the key and value as a two-element array parameters.
*/
VALUE
rbtree_reverse_each_from_key(VALUE self, VALUE key)
{
    dnode_t* node = dict_lookup(DICT(self), TO_KEY(key));
    rbtree_each_arg_t each_arg;
    if (node == NULL) { return self; };
    ...
    each_arg.func = each_i;
    each_arg.arg = node;
    return rb_ensure(rbtree_reverse_each_starting_with_body, (VALUE)&each_arg,
                     rbtree_each_ensure, self);
}
...
void Init_rbtree()
{
    ...
    rb_define_method(MultiRBTree, "each_from_key", rbtree_each_from_key, 1);
    rb_define_method(MultiRBTree, "reverse_each_from_key", rbtree_reverse_each_from_key, 1);
